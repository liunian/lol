...
function scrollShowSchedule() {
	let $schedule = document.querySelector('.schedule__running') ||
		document.querySelector('.schedule__pendding');
	if (!$schedule) return;

	const $parent = $schedule.parentElement;
	const pTop = $parent.getBoundingClientRect().top;
	const sTop = $schedule.getBoundingClientRect().top;
	$parent.scrollTop = sTop - pTop;
}

function generateSchedule(matches, onlyTeam) {
	const list = onlyTeam ?
	 matches.filter(i => i[1] === onlyTeam || i[2] === onlyTeam) :
	Array.from(matches);
	...
	return list
		...
		.map(i => {
			...
			return `<li class="schedule schedule__${cls}">
				<span class="time">${i[3].format(DATE_TIME_FMT)}</span>
				<span class="team">${i[1]}</span>
				<span class="score">${i[4]}</span>
				<span class="team">${i[2]}</span>
			</li>`;
		})
		.join('');
}

function fillTeamsSelect(teams) {
	const frag = document.createDocumentFragment();
	frag.appendChild(new Option('只看战队', ''));
	teams.forEach(t => {
		frag.appendChild(new Option(t[0], t[0]));
	});
	return frag;
}