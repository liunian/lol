...
function sortRanking(rankings, matches) {
	const compareRanking = function(a, b) {
		// 先按大场胜率排
		const matchWinRate = b.matchWin / b.matchCount - a.matchWin / a.matchCount;
		if (matchWinRate !== 0) return matchWinRate;

		// 再按小场胜率排
		const gameWinRate = b.gameWin / (b.gameWin + b.gameLoose) - a.gameWin / (a.gameWin + a.gameLoose);
		if (gameWinRate !== 0) return gameWinRate;
...
function getDetailCell(teamX, x, teamY, y, match) {
	...
	if (!match[4]) {
		...
		return `<td class="cell__pending">${text}</td>`;
	}
...
function handleHoverTableCell(cell, table) {
	...
	const colIndex = [].slice.call(tr.cells, 0).indexOf(cell);
	const rows = tr.parentElement.children;
	const rowIndex = [].slice.call(rows, 0).indexOf(tr);
	...
	const hTop = trRect.top + scrollTo;
	const hLeft = trRect.left + scrollLeft;

	const lines = getCrossLines();
	lines.h[0].style.top = hTop + 'px';
	lines.h[0].style.left = hLeft + 'px';
	lines.h[1].style.top = hTop + 'px';
}

function handleLeaveTable(table) {
	const crossLines = getCrossLines();
	for (let dir in crossLines) {
		const lines = crossLines[dir];
