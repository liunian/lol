...
function sortRanking(rankings, matches) {
	const compareRanking = function(a, b) {
		// 如果其中一队没比赛过，那么看另一队的胜负场差
		if (a.matchCount === 0) {
			if (b.matchCount === 0) return 0;
			return b.matchWin - b.matchLoose;
		} else if (b.matchCount === 0) {
			return a.matchLoose - a.matchWin;
		}

		// 先按大场胜率排
		const matchWinRate = (b.matchWin / b.matchCount || 0) - (a.matchWin / a.matchCount || 0);
		if (matchWinRate !== 0) return matchWinRate;

		// 再按小场胜率排
		const gameWinRate = (b.gameWin / (b.gameWin + b.gameLoose) || 0) - (a.gameWin / (a.gameWin + a.gameLoose) || 0);
